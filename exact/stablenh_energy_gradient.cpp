...
double exact::stablenh::energy(const Store& store, Eigen::VectorXd& Fvec, const Eigen::VectorXi& bone_or_muscle){
	...
	{	
		...
		for(int i=0; i<store.T.rows(); i++){
			...
			sNHpriv += W*store.rest_tet_vols[t];
		}
...
double exact::stablenh::energy(const Store& store, Eigen::VectorXd& Fvec){
	double stableNHEnergy = 0;
...
void exact::stablenh::gradient(const Store& store, const Eigen::VectorXd& Fvec, Eigen::VectorXd& grad, const Eigen::VectorXi& bone_or_muscle){
	grad.setZero();
	...
	for(int m=0; m<store.bone_tets.size(); m++){
		...
		grad.segment<9>(9*m) +=  store.bone_vols[m]*tet_grad;

	}
}
void exact::stablenh::gradient(const Store& store, const Eigen::VectorXd& Fvec, Eigen::VectorXd& grad){
	grad.setZero();
...
void exact::stablenh::hessian(const Store& store, const VectorXd& Fvec, Eigen::SparseMatrix<double, Eigen::RowMajor>& hess, const Eigen::VectorXi& bone_or_muscle){
	hess.setZero();
		...
		for(int i=0; i<store.muscle_tets[m].size(); i++){
			...
			for(int i=0; i<ddw.rows(); i++){
				for(int j=0; j<ddw.cols(); j++){
					hess_trips.push_back(Trip(9*f_index + i, 9*f_index + j, store.rest_tet_vols[t]*ddw(i,j)));
				}
			}
	...
	for(int m=0; m<store.bone_tets.size(); m++){
			...
			for(int i=0; i<ddw.rows(); i++){
				for(int j=0; j<ddw.cols(); j++){
					hess_trips.push_back(Trip(9*m + i, 9*m + j, store.bone_vols[m]*ddw(i,j)));
				}
			}
...
void exact::stablenh::hessian(const Store& store, const VectorXd& Fvec, Eigen::SparseMatrix<double, Eigen::RowMajor>& hess){
		hess.setZero();
		std::vector<Trip> hess_trips;
		hess_trips.reserve(9*9*store.T.rows());
