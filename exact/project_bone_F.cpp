...
void exact::project_bone_F(Store& store,  VectorXi& bone_or_muscle, Eigen::SparseMatrix<double, Eigen::RowMajor>& mN, Eigen::SparseMatrix<double, Eigen::RowMajor>& mAN){
    ...
    for(int i=0; i<bone_or_muscle.size(); i++){
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+0, 9*bone_or_muscle[i]+0, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+1, 9*bone_or_muscle[i]+1, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+2, 9*bone_or_muscle[i]+2, 1.0));

		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+3, 9*bone_or_muscle[i]+3, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+4, 9*bone_or_muscle[i]+4, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+5, 9*bone_or_muscle[i]+5, 1.0));

		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+6, 9*bone_or_muscle[i]+6, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+7, 9*bone_or_muscle[i]+7, 1.0));
		mNProjectBonesElemsToOneDef_trips.push_back(Trip(9*i+8, 9*bone_or_muscle[i]+8, 1.0));
		
    }
    int maxInd = bone_or_muscle.maxCoeff();
