...
public:
    Rosenbrock(int n_, Mesh* m, Arap* a, UElastic* e, json& j_input, bool test=false) : n(n_) {
    	mesh = m;
        arap = a;
        elas = e;
 ...
 //    VectorXd Full_ARAP_Grad(Mesh& mesh, Arap& arap, UElastic& elas, double E0, double eps){
 //        VectorXd fake = VectorXd::Zero(mesh.red_s().size());
 //        for(int i=0; i<fake.size(); i++){
 //            mesh.red_s()[i] += 0.5*eps;
 //            // mesh.setGlobalF(false, true, false);
 ...
 //    VectorXd WikipediaEnergy_grad(Mesh& mesh, UElastic& elas, double eps){
	//     VectorXd fake = VectorXd::Zero(mesh.red_s().size());
	//     for(int i=0; i<fake.size(); i++){
	//         mesh.red_s()[i] += 0.5*eps;
    ...
    double operator()(const VectorXd& x, VectorXd& grad, bool computeGrad = true)
    {
        ...
        if(computeGrad){
            ...
            timer.start();
            arap->Gradients(*mesh);
            VectorXd arapgrad = alpha_arap*mesh->N().transpose()*arap->Es();
            timer.stop();
